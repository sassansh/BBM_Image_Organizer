...
def parse_filename(filename):
    ...
    try:
        ...
        if 'SEM' in filename:
            SEM_number = 'SEM' + \
                filename.split('SEM')[1].split('.')[0]
        raw_angle = filename.split(' (')[1].split(')')[0]
        angle = ''.join(
            i for i in raw_angle if not i.isdigit())
...
for imagePath in glob.iglob(IMAGES_ROOT_DIRECTORY + '**/*', recursive=True):
    ...
    # Parse filename
    scientific_name_filename, SEM_number_filename, angle_filename = parse_filename(imageFileName)
    ...
    if SEM_number_filename == "" or SEM_number_filename == 'PARSING FAILED':
        ...
        result_original, sem_conf_orignal = cropped_scan(image, "original", reader)
        ...
        if (sem_conf_orignal < MIN_CONFIDENCE):
            print('Poor detection, trying red channel')
            # Perform cropped red OCR
            result_red, sem_conf_red = cropped_scan(image, "red", reader)
        ...
        if (not SEM_exists_in_result):
            ...
            result_original, sem_conf_original = full_scan(image, "original", reader)
            ...
            if (sem_conf_orignal < MIN_CONFIDENCE):
                print('Poor detection, trying red channel')
                result_red, sem_conf_red = full_scan(image, "red", reader)
